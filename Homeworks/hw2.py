...
conn = psycopg2.connect(database="covid", user="aleksandrskiba",
    password="31471", host="localhost", port=5432)

cur = conn.cursor()
# cur.execute("DROP TABLE IF EXISTS patients")
cur.execute("CREATE TABLE IF NOT EXISTS patients (patient_id SERIAL PRIMARY KEY,first_name VARCHAR(64), " +
    "last_name VARCHAR(64), birth_date VARCHAR(64), phone VARCHAR(64), "
    "document_type VARCHAR(64), document_id VARCHAR(64))")
conn.commit()
...
class Patient:
    ...
    def save(self):
        ...
        if self._saved:  # Если пациент уже существует
            ...
            cur.execute(f"UPDATE patients SET first_name = '{self.first_name}', last_name = '{self.last_name}', birth_date = '{self.birth_date}',"
                        f"phone = '{self.phone}', document_type = '{self.document_type}', document_id = '{self.document_id}' WHERE patient_id = {self._row_idx}")
            conn.commit()
...
if __name__ == "__main__":
    ...
    # p.save()
    # p = Patient('Bill', 'Gates', '08-04-1984', '89126456891', 'паспорт', '5162555666')
    # p.save()
